...
L:	c.le.s $f12, $f10	#if i<=2.0 set FCSR flag to 1
	...
	swc1 $f12, 4($sp)	#store i on stack
	sw $a1, 8($sp)		#store n on stack

	jal Taylor_ln		#call Taylor_ln
	swc1 $f0, 12($sp)	#store return value on stack
	...
	lwc1 $f12, 12($sp)	#load Taylor_ln(i, n) from stack and print on screen
	li $v0, 2		
	syscall 		
	...
	lwc1 $f12, 12($sp)	#load and print Taylor_lnRecursive(i, n) from stack
	li $v0, 2		
	syscall 		
	...
	lw $ra, 0($sp)		#load return add form stack
	lwc1 $f12, 4($sp)	#load i from stack
	lw $a1, 8($sp)		#load n from stack
	lwc1 $f0, 12($sp)	#load return value frome stack
	addi $sp, $sp, 16	#free space on stack
